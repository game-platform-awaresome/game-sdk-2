module platform {
	...
	// 悟饭sdk
	//--------------------------------------------------
	export class SdkWuFan extends SdkBase {
		public constructor() {
			super(WF);
			this._channleId = 10015;
			this._appId = "16001797";
		}
		public getScripts(): string[] {
			return ["https://sdk.5fun.cn/static/ios_h5_sdk/wufanh5sdk.js"];
		}
		public start(): boolean {
			if (!(window as any).WuFan) return false;
			(window as any).WuFan.config({ appkey: this._appId, gameUrl: "https://cdn0.myh5.90wmoyu.com/index.wf.html", notifyUrl: "http://pay.90wmoyu.com/platform_wf/pay_wf.php" });
			(window as any).WuFan.login(this.callback, this)
			return true;
		}
		private callback(tokenObj) {
			...
			//do something
			(window as any).WuFan.buoy();
			(window as any).WuFan.desktop();
		...
		public submitExtraData(
			...
			switch (dataName) {
				case DATA_CREATE_ROLE:
					...
					(window as any).WuFan.createRole(obj);
			}
		}
