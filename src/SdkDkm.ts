module platform {
    ...
    // 贪玩sdk
    //--------------------------------------------------
    export class SdkDkm extends SdkBase {

        public constructor() {
            super(DKM);
            this._channleId = 10025;
            this._appId = '647'
        }
        ...
        public submitExtraData(
            ...
            switch(dataName){
                case DATA_CREATE_ROLE:
                    (window as any).AKSDK.logCreateRole(serverId, serverName, this._roleId, gameRoleName, gameRoleLevel,new Date().getUTCSeconds())
                break;
                case DATA_ENTER_GAME:
                    (window as any).AKSDK.logEnterGame(serverId, serverName, this._roleId, gameRoleName, gameRoleLevel,new Date().getUTCSeconds())
                break;
                case DATA_LEVEL_UP:
                    (window as any).AKSDK.logRoleUpLevel(serverId, serverName, this._roleId, gameRoleName, gameRoleLevel,new Date().getUTCSeconds())
                break;
            }
        }
